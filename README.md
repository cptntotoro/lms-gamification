...
### Основные эндпоинты API (март 2026)
...
API использует **заголовочную аутентификацию** без паролей. Каждый запрос должен содержать два обязательных
HTTP-заголовка:

- `X-User-Id` — идентификатор пользователя из LMS (строка)
- `X-Role` — роль пользователя (одна из: `STUDENT`, `TEACHER`, `METHODIST`, `ADMIN`)

Если заголовки отсутствуют, запрос обрабатывается как анонимный и может быть отклонён (401 Unauthorized) для защищённых
эндпоинтов.
...
|-------------|---------------------------------------------------------------------------------------------|------------------------------------------------------------|
| `STUDENT`   | Обычный студент. Может просматривать только свои данные.                                    | `/api/v1/users/*`, `/api/v1/leaderboard/*`, `/api/v1/me/*` |
| `TEACHER`   | Преподаватель. В будущем — доступ к лидербордам своих курсов.                               | (пока не реализовано, но роль принимается)                 |
| `METHODIST` | Методист. В будущем — доступ к аналитике по курсам.                                         | (пока не реализовано, но роль принимается)                 |
| `ADMIN`     | Администратор системы. Полный доступ ко всем эндпоинтам `/api/admin/*`, `/api/analytics/*`. | Все административные и аналитические API                   |

#### Проверка владельца данных

Для эндпоинтов, возвращающих данные конкретного пользователя (например, `/api/v1/users/{userId}`), используется
аннотация `@PreAuthorize("#userId == authentication.principal.userId")`. Это гарантирует, что даже если пользователь с
ролью `STUDENT` попытается обратиться к данным другого студента, запрос будет отклонён с кодом 403.
...
```bash
curl -X GET "http://localhost:8080/api/v1/users/alex123" \
  -H "X-User-Id: alex123" \
  -H "X-Role: STUDENT"
```
...
- Нажмите кнопку Authorize (замочек)
- Введите в поля:
    - X-User-Id: alex123
    - X-Role: ADMIN (если нужен доступ к админке)
- Нажмите Authorize и выполните запрос.
