...
|-----------------------------|--------------------------------------|-------------------------------------------------------------------------|----------------------|
| Пользователи                | `/api/v1/admin/transactions`                   | Просмотр списка студентов, очков, уровней, детальная карточка + история | ROLE_ADMIN           |
| Типы событий (CRUD)         | `/api/v1/admin/event-types`             | Создание, редактирование, деактивация типов событий                     | ROLE_ADMIN           |
| История транзакций студента | `/api/v1/admin/transactions/{id}/transactions` | Полная история начислений конкретного студента                          | ROLE_ADMIN           |
...
- **Детали конкретного студента**  
  GET `/api/v1/admin/transactions/{userId}`  
  → полная информация + ссылка на историю транзакций

- **История начислений студента**  
  GET `/api/v1/admin/transactions/{userId}/transactions`  
