...
public interface GroupRepository extends JpaRepository<Group, UUID> {
    ...
    /**
     * Получить UUID группы по её внешнему идентификатору из LMS и идентификатору курса
     *
     * @param groupId  Идентификатор группы из LMS
     * @param courseId Идентификатор курса из LMS
     * @return Optional с UUID группы или пустой, если не найден
     */
    @Query("SELECT g.uuid FROM Group g WHERE g.groupId = :groupId AND g.course.courseId = :courseId")
    Optional<UUID> findUuidByGroupIdAndCourseCourseId(
