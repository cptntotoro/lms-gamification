 ...
 *
 * Required configuration (set before loading this script):
 *
...
(function() {
    ...
    let config = window.GamificationWidgetConfig || {};
    const apiBaseUrl = config.apiBaseUrl;
    const userId = config.userId;
    let updateIntervalMs = (config.updateIntervalMs && !isNaN(config.updateIntervalMs))
    ...
    // ---------- State ----------
    let globalData = null;          // UserGlobalCourseGroupDto (global progress)
    ...
    async function fetchGlobalProgress() {
        ...
        return data; // UserGlobalCourseGroupDto
    }
    ...
    async function fetchUserCourses() {
        ...
        const data = await response.json();
        return data; // UserCoursesResponseDto
    }
    ...
    async function fetchLeaderboard(courseId, page = 0, size = DEFAULT_PAGE_SIZE) {
        ...
        const data = await response.json();
        return data; // UserCourseGroupLeaderboardDto
    }

    // Refresh all data (global + courses) and optionally leaderboard if tab active
    async function refreshAllData() {
        ...
        try {
            // Parallel fetch global and courses
            ...
            // Update floating button with quick info
            updateFloatingButtonInfo();

            // If popup is open, re-render current tab
            if (isOpen) {
                renderCurrentTab();
            }

            // If leaderboard tab is active and we have a selected course, refresh leaderboard as well
    ...
    // Update the floating button (minimized view)
    ...
    function showErrorMessage(msg) {
        ...
        if (contentDiv) {
            ...
            // Keep visible for a while, but don't spam
            ...
            btn.addEventListener("click", (e) => {
                ...
                if (cid) {
                    ...
                    refreshLeaderboardForSelectedCourse().then(() => {
                        // after leaderboard loaded, update tab title maybe
    ...
    // Build UI elements (floating button + popup panel)
